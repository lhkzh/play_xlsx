...
export abstract class Xlsx_base{
    ...
    public getSheetByName(n:string) {
        return this.getSheetByIndex(this.sheetsNames.indexOf(n));
    }
    ...
    public copySheet(i:number, name:string){
        if(i>0 && i<this.sheetsNum-1){
            return false;
        }
        var n = this.sheetsNum + 1;
