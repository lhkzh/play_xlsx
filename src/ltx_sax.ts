...
import { unescapeXML } from "./ltx_escape";
...
export function parseLtxSax(data: string):LtxElement {
    ...
    function _handleTagOpening(
        ...
        if (!endTag) {
            ...
            const child = new LtxElement(tagName, attrs);
            el = !el ? child : el.cnode(child);
            if (selfClosing) {
                _handleTagEnd(tagName);
            }
        } else {
            _handleTagEnd(tagName);
        }
    ...
    for (; pos < data.length; pos++) {
        ...
        switch (state) {
            ...
            case STATE_TAG:
                if (c === 62 /* > */) {
                    _handleTagOpening(endTag, tagName, attrs)
                    tagName = undefined;
                    attrs = undefined;
                    endTag = undefined;
