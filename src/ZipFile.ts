/**
 * @author https://github.com/Teal/TUtils/blob/master/src/zipFile.ts
 */
 ...
 */
 export function writeFile(path: string, data: string | Buffer, overwrite = true): boolean {
	try {
		fs.writeFileSync(path, data, overwrite ? undefined : { flag: "wx" })
	} catch (e) {
		switch (e.code) {
			case "ENOENT":
 ...
 * @param options 遍历的选项
 */
 export function walk(path: string, options, /**@internal */ _stats?) {
	if (!_stats || _stats.isDirectory()) {
		...
		try {
			...
			if (e.code === "ENOTDIR") {
				let stats
				try {
					stats = (options.follow !== false ? fs.statSync : fs.lstatSync)(path)
				} catch (e) {
 ...
 * @param pathstr 相关的路径
 */
 export function ensureDirExists(pathstr: string) {
	return createDir(path.dirname(path.resolve(pathstr)))
}
/**
 * 合并多个路径
 * @param paths 要处理的路径
 ...
 */
 export function joinPath(...paths: readonly string[]) {
	...
	return pathstr === "." ? "" : path.sep === "/" || path.isAbsolute(pathstr) ? pathstr : pathstr.replace(/\\/g, "/")
}
/**
 * 获取指定路径对应的相对路径
 * @param base 要使用的基路径
 * @param pathstr 要处理的路径
 * @returns 返回以 `/` 为分隔符的相对路径，路径末尾多余的分隔符会被删除
 * @example relativePath("foo/goo/hoo", "foo/goo/relative") // "../relative"
 */
 export function relativePath(base: string, pathstr: string) {
	pathstr = path.relative(base, pathstr)
 ...
 */
 export function getExt(paths: string) {
	if (paths.endsWith("/") || paths.endsWith(path.sep)) {
		return ""
	}
 ...
 export class ZipFile {
      ...
      */
     addFile(path: string, zipName = path, data = fs.readFileSync(path), stats = fs.statSync(path)) {
     ...
     static fromFile(path: string) {
         return ZipFile.fromBuffer(fs.readFileSync(path))
     }
